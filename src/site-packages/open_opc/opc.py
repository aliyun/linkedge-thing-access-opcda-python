...
if opc_mode == 'open' and not pyro_found:
   print("Pyro4 module required when using Open protocol mode (https://pypi.python.org/pypi/Pyro4)")
   exit()
...
if action == 'sessions':
   print('  %-32s %-38s %-18s %-18s' % ('OPC Server', 'Client Object', 'Start Time', 'Last Transaction'))
   try:
      for server, guid, init_time, tx_time in OpenOPC.get_sessions(open_host, open_port):
         print('  %-32s %-38s %-18s %-18s'  % (server, guid, time2str(init_time), time2str(tx_time)))
   except:
