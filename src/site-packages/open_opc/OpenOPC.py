...
class client():
               ...
               if tag in tag_value:
                  if (not sync and len(valid_tags) > 0) or (sync and tag_error[tag] == 0):
                     ...
                     if type(value) == pywintypes.TimeType:
                        value = str(int(round(value.timestamp() * 1000)))
   ...
   def ping(self):
      ...
      try:
         ...
         if not self._opc.CurrentTime:
            return False

         opc_serv_time = time.mktime(self._opc.CurrentTime.timetuple())
         ...
         error_msg = 'ping: %s' % self._get_error_str(err)
         raise OPCError(err)
   ...
   def _update_last_time(self):
      ...
      if self._open_serv:
         self._open_serv._last_times[self._open_guid] = time.time()
